...
export function createElement (
  ...
  if (Array.isArray(data) || isPrimitive(data)) {
    // data长什么样子——data为对象。
    // 如果data为Array类型或基本类型，则认为无data，此时传递的事实上作为children处理
...
由于`Virtual DOM`实际上是一个树状结构，每一个VNode可能会有若干个子节点，这些子节点也应该是VNode类型，而`_createElement`方法接收的参数`children`是任意类型的（发生在手写render时），因此需要将它规范成VNode类型。
...
#### VNode的创建
回到`_createElement`方法内，在对`children`做规范化处理后，接下来的工作是创建`VNode`实例。
...
if (typeof tag === 'string') {
  ...
  if (config.isReservedTag(tag)) {
    ...
    // 会依次通过普通命名、camelize、capitalize几种命名风格检查tag是否为注册的组件名
    // 创建组件类型的VNode
    vnode = createComponent(Ctor, data, context, children, tag)
  } else {
    vnode = new VNode(
