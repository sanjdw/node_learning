...
可以发现这里的`render`方法的`createElement`参数对应的正是这里的`vm.$createElement`方法。

`vm.$createElement`是在Vue实例初始化阶段（`_init(vm)`）通过`initRender(vm)`定义在Vue实例上的：
```js
// src/core/instance/render.js
...
可以看到除了`$createElement`方法，实例上还定义了`vm._c`方法——这个方法是在`render`方法来自模板编译case使用的，而`$createElement`是在`render`方法来自手写情况下使用的，它们都是通过调用`createElement`实现的，区别在于最后一个参数分别为`true`和`false`：
```js
...
`_createElement`是真正用于创建vnode的方法，它接收了几个描述vnode信息的参数，并在创建vnode之前先做了`children`的规范化处理。
