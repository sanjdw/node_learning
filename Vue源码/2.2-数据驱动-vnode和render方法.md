上一节提到`_render`方法用于将实例转换为`vnode`对象，`_update`将生成的`vnode`渲染成真实的DOM。本文分析`vnode`的生成。

在此之前，我们先来搞明白`vnode`是什么。

### 1.vnode是什么
由于浏览器标准把真实DOM设计得非常复杂，频繁地更新DOM会产生性能问题，`virtual DOM`也就有了舞台——虚拟DOM是用于描述DOM节点的原生JavaScript对象。

而在Vue.js中，`vnode`就是用来描述`virtual DOM`的：
```js
...
### 2.生成vnode的_render方法
`_render`是实例的私有方法，它在`renderMixin(Vue)`执行时被挂载到`Vue`原型上，这一步是在构建平台确定时就已经确定的：
```js
...
至于`vm.$createElement`，与Vue.js的官方文档中介绍的手写`render`函数提供的参数`createElement`：
![render](https://pic.downk.cc/item/5f549704160a154a670e7a4e.jpg)

做比较，即可发现手写`render`方法中提供的`createElement`对应的正是`vm.$createElement`方法。

而`$createElement`是在`_init(vm)`阶段（创建Vue实例时）执行`initRender(vm)`时定义在实例上的：
```js
// src/core/instance/render.js
...
#### 2.2 vnode的创建
回到`_createElement`方法主流程中，在对`children`做规范化处理之后，接下来的工作就是创建`vnode`实例了：
```js
let vnode
if (typeof tag === 'string') {
  // 字符串类型
...
### 总结
至此，我们大致了解了`_render`方法创建`vnode`的过程。每个`vnode`都有`children`，而`children`的每个元素又是一个`vnode`，这样就形成了一个`VNode Tree`，它可以简单的描述对应的`DOM Tree`。

接下来将分析把这个`vnode`渲染成真实的`DOM`的过程——`vm._update(vnode)`。
