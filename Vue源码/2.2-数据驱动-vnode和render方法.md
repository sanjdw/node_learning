上一节提到vue实例的`_render`方法用于~~将实例转换为vnode~~生成vue实例对应的vnode，`_update`方法将生成的vnode渲染成真实的DOM，本文分析vnode的生成。

在此之前，我们先来搞明白vnode是什么。
...
### 2.生成vnode的_render方法
`_render`是vue实例的私有方法，它是在`renderMixin(Vue)`时被挂载到`Vue`原型上（这一步是在构建平台确定时就已经确定的）：
```js
...
`render`方法来源于`options`：它可能是开发者手写的`render`，也可能是由模板`template`编译成的`render`。由模板生成`render`的case，后面会单独做分析，在这里暂讨论手写`render`的情况。

至于`vm.$createElement`，将它与官方文档中介绍的通过渲染函数创建vue实例所提到的`createElement`：
![render](https://pic.downk.cc/item/5f549704160a154a670e7a4e.jpg)

做比较，即可发现那里提到的`createElement`对应的正是这里的`vm.$createElement`方法。

`vm.$createElement`是在vue实例初始化执行`_init(vm)`方法内的`initRender(vm)`时定义在实例上的：
```js
// src/core/instance/render.js
...
可以看到除了`$createElement`方法，实例上还定义了`vm._c`方法——这个方法是在`render`方法来自模板编译生成情况下使用的，而`$createElement`是在`render`方法来自手写情况下使用的，它们都是通过调用`createElement`实现的，区别在于最后一个参数分别为`true`和`false`：
```js
...
可以看到`createElement`只是做了一层封装，本质还是对`_createElement`的调用：
```js
// src/core/vdom/create-element
...
#### 2.2 vnode的创建
回到`_createElement`方法主流程中，在对`children`做规范化处理之后，接下来的工作就是最关键的创建vnode：
```js
let vnode
if (typeof tag === 'string') {
  // 字符串类型
...
在这个过程中，要清醒的是，**实例vm和vnode是什么关系**？vnode是实例调用自身的`_render`方法生成的:
```js
let vnode = render.call(vm._renderProxy, vm.$createElement)
```

1. 开发者声明实例时提供的`render`（可能来自模板编译，也可能是来自手写）描述了实例的数据、DOM结构、事件等
2. `render`通过将这些描述信息作为参数传递给`$createElement`生成真正的vnode
