...
在通过组件vnode创建对应的根节点实例之后，将它赋给`vnode.componentInstance`来保留对子组件实例的引用。然后通过子组件实例`ch`的`$mount`方法挂载子组件，关于`ch.$mount`的逻辑就再次回到之前的流程中去。也就是说组件的`update`操作本质是通过创建子组件实例，借助子组件实例的`$mount`挂载实现。

在继续分析`createComponent`的`init`钩子后的流程之前，我们先回顾一下Vue实例的`_init`方法：
```js
