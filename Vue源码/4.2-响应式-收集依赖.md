...
以上，`dep`可以看做订阅者，`watcher`看做发布者，`dep`订阅了`watcher`：
- `dep.subs`中维护了它所订阅的`watcher`（一个响应式数据可能在多个Vue实例上访问，也可能在一个Vue实例的模板、计算属性和watch中使用
- `wathcer.newDeps`/`watcher.newDepIds`维护了订阅自身的所有`dep`

一个`wathcer`对应了一个Vue实例。

回到`watcher.get`中，除了通过`value = this.getter.call(vm, vm)`触发了响应式数据的`getter`中`dep`对`wacther`的订阅、`watcher`对`dep`的收集逻辑之外，还有以下步骤：
...
### 总结
这一小节对Vue响应式数据的依赖收集过程的一些细节做了分析，收集依赖的目的是在这些响应式数据发生变化触发它们的`setter`时方便通知到对应的订阅者`dep`去做相应的视图更新的逻辑处理，这个过程叫派发更新。
