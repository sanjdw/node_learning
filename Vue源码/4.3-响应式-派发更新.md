...
这里会比较响应式数据新旧值是否相同，如果相同则不会派发更新；对于被重置后的响应式数据变量，需要再次处理为响应式数据。最后是通过`dep.notify()`通知他所订阅的所有发布者`watcher`，回到`Dep.notify`：
```js
class Dep {
  //...
  notify () {
...
`dep.notify`的逻辑就是遍历其`subs`中所维护的`watcher`，调用对应的`update`：
```js
class Watcher {
  // ...
  update () {
