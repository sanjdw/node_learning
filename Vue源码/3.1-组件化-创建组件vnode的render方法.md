...
在上一小节中我们刚分析过，`createElement`方法在创建vnode时有这样一段逻辑：
```js
...
Vue的组件系统允许我们直接在模板中使用组件或者在手写渲染函数时以组件作为`tag`参数传给`createElement`，比如：
```js
import Vue from 'vue'
...
`_createElement`这里传入的`tag`对应的是一个`App`对象，它对应的是一个组件：
![App](https://pic.downk.cc/item/5f57ac87160a154a67bb9b49.jpg)

所以`_createElement`的逻辑会走到`createComponent(tag, data, context, children)`逻辑中去：
```js
// src/core/vdom/create-component.js
function createComponent (Ctor, data, context, children, tag) {
  // 根组件的context.$options._base为Vue
