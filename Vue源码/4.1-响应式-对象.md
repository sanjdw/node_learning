...
这里主要是对实例的`props`、`data`、`methods`、`data`、`computed`和`watch`做了初始化操作，下面具体来看一下这几个属性的初始化工作。
...
总之这里可以看到，`props`和`data`在实例的初始化过程中都被处理为了响应式对象，下面来看实现它们的`observe`和`defineReactive`，以及访问代理`proxy`。
...
对于数组的处理，`observeArray`又递归调用了`observe`方法；而`walk`则遍历原始对象的`key`调用`defineReactive`。现在，一切又指向了`defineReactive`。
