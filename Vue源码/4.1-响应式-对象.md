...
`observeArray`递归调用了`observe`方法，而`walk`则遍历原始对象的`key`调用`defineReactive`。

### 5. defineReactive
`defineReactive`用来将一个对象变成响应式的：
```js
...
`defineReactive`函数首先创建了一个`Dep`实例，接着拿到`obj`的属性描述符，然后对子对象递归调用`observe`方法，这样就保证了被监测的数据的所有子属性也是响应式的。最后利用`Object.defineProperty`去给`obj`的属性`key`添加`getter`和`setter`，而关于`getter`和` setter`的具体实现，会在下一节分析。
