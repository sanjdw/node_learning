...
- 根据`vnode.tag`创建对应的`DOM`元素（`nodeOps.createElement`）来初始化`vnode.elm`
- 调用`createChildren`创建子`vnode`对应的DOM
...
- 开发环境下检查`key`
- 递归调用`createElm`方法创建各`vnode`对应的DOM
...
由于是深度优先遍历，子`vnode`会优先调用`insert`，所以整个`vnode`树的插入顺序是先子后父。
...
至此我们从主线上把`render`如何生成`vnode`、如何根据`vnode`创建DOM的过程分析完毕。本节分析了初始化`Vue`实例到渲染DOM的简单的场景，数据变化引起的`update`、组件生成`vnode`等过程还要在后续的代码分析中解答。
