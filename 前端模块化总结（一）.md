...
```js
// AMD
define(['./math', './count-down'], function(math, count) { 
  // 依赖必须一开始就写好
...
CMD推崇就近依赖，在需要用到某个模块的时候再去`require`

___
## ES 6模块
`ES 6`在语言标准的层面上，实现了模块功能，**旨在成为浏览器和服务器通用的模块解决方案**。其模块功能主要由两个命令构成：`export`和`import`。`export`命令用于规定模块的对外接口，`import`命令用于加载其他模块。

在ES 6标准中，
1. `export`只支持对象形式的导出，不支持值的导出
2. `export default`命令用于指定模块的默认输出，只支持值导出，且只能指定一个
